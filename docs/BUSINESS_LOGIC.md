...
```
用戶輸入用戶名 → 系統建立用戶 → 所有單字預設在 P0，等級與分類為 null
```
...
**目的**: 讓有基礎的用戶可以跳過簡單等級，從適合自己的難度開始學習。

**流程**:
```
1. 用戶進入程度分析
2. 系統從每個等級（A1.1 - B2.2）各隨機挑選 10 個單字
3. 所有測驗題型均為 Reading Lv1（看英文選中文）
4. 用戶完成 80 題測驗後，前端根據答題結果計算合適等級
5. 前端提交 level_order，系統更新用戶的 current_level_id
6. 同時將 current_category_id 設定為 1 (Function Words)
```
...
1. 用戶進入學習模式
2. 系統檢查是否可以學習：
   - 是否已完成程度分析（current_level/current_category != null）？
   - 今日學習數 < 50？
   - P1 池即將可用的單字 < 10？
   - P0 池有單字可學？
3. 若可以，取出 5 個 P0 單字
